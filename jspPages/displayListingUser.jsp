...
//Verify listing has not ended



if(endbid == -1 && current != 0)//If bid is active and there is a current bid
{
	...
	System.out.println("With current");
	System.out.println("Current: " + ct.toString());
	System.out.println("End: " + end.toString());
	
	
	int status=end.compareTo(cd);
	
	
	if(status<=0) //If currentTime is newer or equal to endTime
	{
		if(current >= reserve)//If the current bid meets the reserve, declare winner
		{
			endbid = current;
			stm = con.createStatement();
			String query4 = "UPDATE Auction SET End_Bid = ('"+endbid+"') WHERE Post_ID = '"+postID+"'";
			stm.executeUpdate(query4);
			db.closeConnection(con);
			response.sendRedirect("displayListingOver.jsp");
			//redirect to listing display without bidding functionality
		}
		else //current bid does not meet the reserve
		{
			sesh.setAttribute("res","");
			db.closeConnection(con);
			response.sendRedirect("displayListingNoSale.jsp");
		}
	}
}

if(endbid == -1 && current == 0)//If bid is active and noone has won
{

	Date date = new Date();
	java.sql.Date cd = new java.sql.Date(date.getTime());
	java.sql.Timestamp ct = new java.sql.Timestamp(cd.getTime());
	int status=end.compareTo(ct);
...
else if(endbid != -1)//If bid is over and a winner has been declared
{
	db.closeConnection(con);
	response.sendRedirect("displayListingOver.jsp");
}
...
String lowbid = "";
System.out.println(sesh.getAttribute("low").toString());
if(!(lowbid.equals(sesh.getAttribute("low").toString())))
{
	lowbid = "*Bid was too low!*";
	sesh.setAttribute("low","");
}

String auto = "";
System.out.println(sesh.getAttribute("auto").toString());
if(!(lowbid.equals(sesh.getAttribute("auto").toString())))
{
	auto = "Auto Bid already set!";
...
<form method="post" action="bidVerify.jsp">
	...
	<tr>
		<td>
			<form method="post" action="autoBid.jsp"> 
				<input value="Set An Auto Bid" type="submit">
			</form>
		</td>
	</tr>
</table>
...
<table>
	<tr>
		<td>
			<form method="post" action="a_showSimilar.jsp"> 
